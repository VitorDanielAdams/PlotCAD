...
import { computeGeoVertices, buildKml, downloadKml } from "../../utils/kml"
...
const KmlExportModal = ({ isOpen, onClose, segments, landName, isClosed }: IKmlExportModalProps) => {
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div>
            ...
            {landName && (
              <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[300px]">{landName}</p>
            )}
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          {!isClosed && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-yellow-50 border border-yellow-200 rounded-md">
              ...
              <p className="text-xs text-yellow-700">
                O polígono não está fechado. O KML será gerado com os vértices disponíveis.
              </p>
            </div>
          )}

          <p className="text-xs text-gray-500">
            Informe as coordenadas geográficas do <strong>primeiro vértice</strong> do levantamento
            (marco inicial). Os demais vértices serão calculados a partir dos rumos e distâncias.
          </p>
          ...
          <p className="text-xs text-gray-400">
            {segments.length} segmento{segments.length !== 1 ? "s" : ""} · o arquivo será salvo como <span className="font-mono">.kml</span> compatível com Google Earth, QGIS e AutoCAD Map.
          </p>
        </div>
