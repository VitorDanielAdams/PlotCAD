...
export const CARDINAL_OPTIONS: CardinalDirection[] = [
	"N",
	"S",
	"E",
	"O",
...
const BEARING_REGEX = new RegExp(
	`^${DIRECTION_PATTERN}(\\d{4,6})${DIRECTION_PATTERN}$`,
	"i",
);
...
export function useLandDraw() {
	...
	const updateSegment = useCallback(
		(id: string, field: keyof ISegment, value: string | number) => {
			setSegments((prev) =>
				prev.map((s) => {
					if (s.id !== id) return s;
	...
	const toggleSegment = useCallback(
		(id: string) => setOpenSegmentId((prev) => (prev === id ? null : id)),
		[],
	);
